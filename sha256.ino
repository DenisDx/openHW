...
//DENIS: WARNING! IT NEEDS 350B RAM
//WE COULD put k[] into progmem saving 256b global memory (but it will need more for work)
...
int SHA256(const uint8_t data_buffer[], size_t len, uint8_t *res) 
{
  SHA256_CTX ctx;
  ctx.datalen = 0;
  ctx.bitlen = 512;
  ...
  sha256_init(&ctx);
  sha256_update(&ctx, data_buffer, len);
